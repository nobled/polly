; RUN: opt -polly-print -polly-detect-conditions -disable-output < %s | FileCheck %s
; RUN: opt -polly-codegen -polly-detect-conditions < %s | lli
...
; CHECK: for (s1=0;s1<=20;s1++) {
; CHECK:     S1(s1);
; CHECK:       S2(s1);
; CHECK: }
; CHECK: for (s1=21;s1<=512;s1++) {
; CHECK:     S0(s1);
; CHECK:       S2(s1);
; CHECK: }

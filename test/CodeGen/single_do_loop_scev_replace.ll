...
; RUN: if `%opt -help | grep -q "OpenSCoP"` ; then %opt -polly-import -polly-import-dir=`dirname %s` -polly-codegen < %s | lli ; fi
; RUN: if `%opt -help | grep -q "OpenSCoP"` ; then %opt -polly-import -polly-import-dir=`dirname %s` -polly-import-postfix="_reverse" -polly-codegen < %s | lli ;  fi

; RUN: opt  -indvars -polly-scop-detect -print-top-scop-only -polly-print-scop -analyze  %s | FileCheck %s
; XFAIL: *
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128-n8:16:32:64"
target triple = "x86_64-unknown-linux-gnu"

%struct._IO_FILE = type { i32, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, %struct._IO_marker*, %struct._IO_FILE*, i32, i32, i64, i16, i8, [1 x i8], i8*, i64, i8*, i8*, i8*, i8*, i64, i32, [20 x i8] }
%struct._IO_marker = type { %struct._IO_marker*, %struct._IO_FILE*, i32 }

@r = common global [8000 x double] zeroinitializer, align 32 ; <[8000 x double]*> [#uses=4]
@p = common global [8000 x double] zeroinitializer, align 32 ; <[8000 x double]*> [#uses=4]
@A = common global [8000 x [8000 x double]] zeroinitializer, align 32 ; <[8000 x [8000 x double]]*> [#uses=4]
@s = common global [8000 x double] zeroinitializer, align 32 ; <[8000 x double]*> [#uses=6]
@stderr = external global %struct._IO_FILE*       ; <%struct._IO_FILE**> [#uses=8]
@.str = private constant [8 x i8] c"%0.2lf \00", align 1 ; <[8 x i8]*> [#uses=1]
@q = common global [8000 x double] zeroinitializer, align 32 ; <[8000 x double]*> [#uses=4]

define void @init_array() nounwind inlinehint {
bb.nph6.split.us:
  br label %bb.nph.us

bb3.us:                                           ; preds = %bb1.us
  %indvar.next8 = add i64 %indvar7, 1             ; <i64> [#uses=2]
  %exitcond9 = icmp eq i64 %indvar.next8, 8000    ; <i1> [#uses=1]
  br i1 %exitcond9, label %return, label %bb.nph.us

bb1.us:                                           ; preds = %bb.nph.us, %bb1.us
  %indvar = phi i64 [ 0, %bb.nph.us ], [ %indvar.next, %bb1.us ] ; <i64> [#uses=3]
  %scevgep = getelementptr [8000 x [8000 x double]]* @A, i64 0, i64 %indvar7, i64 %indvar ; <double*> [#uses=1]
  %j.01.us = trunc i64 %indvar to i32             ; <i32> [#uses=1]
  %0 = sitofp i32 %j.01.us to double              ; <double> [#uses=1]
  %1 = fmul double %3, %0                         ; <double> [#uses=1]
  %2 = fdiv double %1, 8.000000e+03               ; <double> [#uses=1]
  store double %2, double* %scevgep, align 8
  %indvar.next = add i64 %indvar, 1               ; <i64> [#uses=2]
  %exitcond = icmp eq i64 %indvar.next, 8000      ; <i1> [#uses=1]
  br i1 %exitcond, label %bb3.us, label %bb1.us

bb.nph.us:                                        ; preds = %bb3.us, %bb.nph6.split.us
  %indvar7 = phi i64 [ %indvar.next8, %bb3.us ], [ 0, %bb.nph6.split.us ] ; <i64> [#uses=5]
  %i.02.us = trunc i64 %indvar7 to i32            ; <i32> [#uses=1]
  %scevgep11 = getelementptr [8000 x double]* @p, i64 0, i64 %indvar7 ; <double*> [#uses=1]
  %scevgep12 = getelementptr [8000 x double]* @r, i64 0, i64 %indvar7 ; <double*> [#uses=1]
  %3 = sitofp i32 %i.02.us to double              ; <double> [#uses=2]
  %4 = fmul double %3, 0x400921FB54442D18         ; <double> [#uses=2]
  store double %4, double* %scevgep12, align 8
  store double %4, double* %scevgep11, align 8
  br label %bb1.us

return:                                           ; preds = %bb3.us
  ret void
}

define void @print_array(i32 %argc, i8** nocapture %argv) nounwind inlinehint {
entry:
  %0 = icmp sgt i32 %argc, 42                     ; <i1> [#uses=1]
  br i1 %0, label %bb, label %return

bb:                                               ; preds = %entry
  %1 = load i8** %argv, align 1                   ; <i8*> [#uses=1]
  %2 = load i8* %1, align 1                       ; <i8> [#uses=1]
  %3 = icmp eq i8 %2, 0                           ; <i1> [#uses=1]
  br i1 %3, label %bb2, label %return

bb2:                                              ; preds = %bb4, %bb
  %indvar = phi i64 [ %indvar.next, %bb4 ], [ 0, %bb ] ; <i64> [#uses=4]
  %scevgep = getelementptr [8000 x double]* @s, i64 0, i64 %indvar ; <double*> [#uses=1]
  %scevgep2 = getelementptr [8000 x double]* @q, i64 0, i64 %indvar ; <double*> [#uses=1]
  %i.01 = trunc i64 %indvar to i32                ; <i32> [#uses=1]
  %4 = load double* %scevgep, align 8             ; <double> [#uses=1]
  %5 = load %struct._IO_FILE** @stderr, align 8   ; <%struct._IO_FILE*> [#uses=1]
  %6 = tail call i32 (%struct._IO_FILE*, i8*, ...)* @fprintf(%struct._IO_FILE* noalias %5, i8* noalias getelementptr inbounds ([8 x i8]* @.str, i64 0, i64 0), double %4) nounwind ; <i32> [#uses=0]
  %7 = load double* %scevgep2, align 8            ; <double> [#uses=1]
  %8 = load %struct._IO_FILE** @stderr, align 8   ; <%struct._IO_FILE*> [#uses=1]
  %9 = tail call i32 (%struct._IO_FILE*, i8*, ...)* @fprintf(%struct._IO_FILE* noalias %8, i8* noalias getelementptr inbounds ([8 x i8]* @.str, i64 0, i64 0), double %7) nounwind ; <i32> [#uses=0]
  %10 = srem i32 %i.01, 80                        ; <i32> [#uses=1]
  %11 = icmp eq i32 %10, 20                       ; <i1> [#uses=1]
  br i1 %11, label %bb3, label %bb4

bb3:                                              ; preds = %bb2
  %12 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %13 = bitcast %struct._IO_FILE* %12 to i8*      ; <i8*> [#uses=1]
  %14 = tail call i32 @fputc(i32 10, i8* %13) nounwind ; <i32> [#uses=0]
  br label %bb4

bb4:                                              ; preds = %bb3, %bb2
  %indvar.next = add i64 %indvar, 1               ; <i64> [#uses=2]
  %exitcond = icmp eq i64 %indvar.next, 8000      ; <i1> [#uses=1]
  br i1 %exitcond, label %bb6, label %bb2

bb6:                                              ; preds = %bb4
  %15 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %16 = bitcast %struct._IO_FILE* %15 to i8*      ; <i8*> [#uses=1]
  %17 = tail call i32 @fputc(i32 10, i8* %16) nounwind ; <i32> [#uses=0]
  ret void

return:                                           ; preds = %bb, %entry
  ret void
}

declare i32 @fprintf(%struct._IO_FILE* noalias nocapture, i8* noalias nocapture, ...) nounwind

declare i32 @fputc(i32, i8* nocapture) nounwind

define void @scop_func() nounwind {
bb.nph8:
  br label %bb

bb:                                               ; preds = %bb, %bb.nph8
  %indvar21 = phi i64 [ 0, %bb.nph8 ], [ %indvar.next22, %bb ] ; <i64> [#uses=2]
  %scevgep24 = getelementptr [8000 x double]* @s, i64 0, i64 %indvar21 ; <double*> [#uses=1]
  store double 0.000000e+00, double* %scevgep24, align 8
  %indvar.next22 = add i64 %indvar21, 1           ; <i64> [#uses=2]
  %exitcond23 = icmp eq i64 %indvar.next22, 8000  ; <i1> [#uses=1]
  br i1 %exitcond23, label %bb.nph.us, label %bb

bb6.us:                                           ; preds = %bb4.us
  store double %6, double* %scevgep15
  %indvar.next10 = add i64 %indvar9, 1            ; <i64> [#uses=2]
  %exitcond13 = icmp eq i64 %indvar.next10, 8000  ; <i1> [#uses=1]
  br i1 %exitcond13, label %return, label %bb.nph.us

bb4.us:                                           ; preds = %bb.nph.us, %bb4.us
  %indvar = phi i64 [ 0, %bb.nph.us ], [ %indvar.next, %bb4.us ] ; <i64> [#uses=4]
  %.tmp.0.us = phi double [ 0.000000e+00, %bb.nph.us ], [ %6, %bb4.us ] ; <double> [#uses=1]
  %scevgep11 = getelementptr [8000 x [8000 x double]]* @A, i64 0, i64 %indvar9, i64 %indvar ; <double*> [#uses=1]
  %scevgep = getelementptr [8000 x double]* @p, i64 0, i64 %indvar ; <double*> [#uses=1]
  %scevgep12 = getelementptr [8000 x double]* @s, i64 0, i64 %indvar ; <double*> [#uses=2]
  %0 = load double* %scevgep12, align 8           ; <double> [#uses=1]
  %1 = load double* %scevgep11, align 8           ; <double> [#uses=2]
  %2 = fmul double %7, %1                         ; <double> [#uses=1]
  %3 = fadd double %0, %2                         ; <double> [#uses=1]
  store double %3, double* %scevgep12, align 8
  %4 = load double* %scevgep, align 8             ; <double> [#uses=1]
  %5 = fmul double %1, %4                         ; <double> [#uses=1]
  %6 = fadd double %.tmp.0.us, %5                 ; <double> [#uses=2]
  %indvar.next = add i64 %indvar, 1               ; <i64> [#uses=2]
  %exitcond = icmp eq i64 %indvar.next, 8000      ; <i1> [#uses=1]
  br i1 %exitcond, label %bb6.us, label %bb4.us

bb.nph.us:                                        ; preds = %bb6.us, %bb
  %indvar9 = phi i64 [ %indvar.next10, %bb6.us ], [ 0, %bb ] ; <i64> [#uses=4]
  %scevgep15 = getelementptr [8000 x double]* @q, i64 0, i64 %indvar9 ; <double*> [#uses=2]
  store double 0.000000e+00, double* %scevgep15, align 8
  %scevgep16 = getelementptr [8000 x double]* @r, i64 0, i64 %indvar9 ; <double*> [#uses=1]
  %7 = load double* %scevgep16, align 8           ; <double> [#uses=1]
  br label %bb4.us

return:                                           ; preds = %bb6.us
  ret void
}

define i32 @main(i32 %argc, i8** nocapture %argv) nounwind {
entry:
  br label %bb.nph.us.i

bb3.us.i:                                         ; preds = %bb1.us.i
  %indvar.next8.i = add i64 %indvar7.i, 1         ; <i64> [#uses=2]
  %exitcond18 = icmp eq i64 %indvar.next8.i, 8000 ; <i1> [#uses=1]
  br i1 %exitcond18, label %bb.i5, label %bb.nph.us.i

bb1.us.i:                                         ; preds = %bb.nph.us.i, %bb1.us.i
  %indvar.i = phi i64 [ 0, %bb.nph.us.i ], [ %indvar.next.i, %bb1.us.i ] ; <i64> [#uses=3]
  %scevgep.i = getelementptr [8000 x [8000 x double]]* @A, i64 0, i64 %indvar7.i, i64 %indvar.i ; <double*> [#uses=1]
  %j.01.us.i = trunc i64 %indvar.i to i32         ; <i32> [#uses=1]
  %0 = sitofp i32 %j.01.us.i to double            ; <double> [#uses=1]
  %1 = fmul double %3, %0                         ; <double> [#uses=1]
  %2 = fdiv double %1, 8.000000e+03               ; <double> [#uses=1]
  store double %2, double* %scevgep.i, align 8
  %indvar.next.i = add i64 %indvar.i, 1           ; <i64> [#uses=2]
  %exitcond17 = icmp eq i64 %indvar.next.i, 8000  ; <i1> [#uses=1]
  br i1 %exitcond17, label %bb3.us.i, label %bb1.us.i

bb.nph.us.i:                                      ; preds = %bb3.us.i, %entry
  %indvar7.i = phi i64 [ %indvar.next8.i, %bb3.us.i ], [ 0, %entry ] ; <i64> [#uses=5]
  %scevgep12.i = getelementptr [8000 x double]* @r, i64 0, i64 %indvar7.i ; <double*> [#uses=1]
  %scevgep11.i = getelementptr [8000 x double]* @p, i64 0, i64 %indvar7.i ; <double*> [#uses=1]
  %i.02.us.i = trunc i64 %indvar7.i to i32        ; <i32> [#uses=1]
  %3 = sitofp i32 %i.02.us.i to double            ; <double> [#uses=2]
  %4 = fmul double %3, 0x400921FB54442D18         ; <double> [#uses=2]
  store double %4, double* %scevgep12.i, align 8
  store double %4, double* %scevgep11.i, align 8
  br label %bb1.us.i

bb.i5:                                            ; preds = %bb.i5, %bb3.us.i
  %indvar21.i = phi i64 [ %indvar.next22.i, %bb.i5 ], [ 0, %bb3.us.i ] ; <i64> [#uses=2]
  %scevgep24.i = getelementptr [8000 x double]* @s, i64 0, i64 %indvar21.i ; <double*> [#uses=1]
  store double 0.000000e+00, double* %scevgep24.i, align 8
  %indvar.next22.i = add i64 %indvar21.i, 1       ; <i64> [#uses=2]
  %exitcond16 = icmp eq i64 %indvar.next22.i, 8000 ; <i1> [#uses=1]
  br i1 %exitcond16, label %bb.nph.us.i12, label %bb.i5

bb6.us.i:                                         ; preds = %bb4.us.i
  store double %11, double* %scevgep15.i
  %indvar.next10.i = add i64 %indvar9.i, 1        ; <i64> [#uses=2]
  %exitcond15 = icmp eq i64 %indvar.next10.i, 8000 ; <i1> [#uses=1]
  br i1 %exitcond15, label %scop_func.exit, label %bb.nph.us.i12

bb4.us.i:                                         ; preds = %bb.nph.us.i12, %bb4.us.i
  %indvar.i6 = phi i64 [ 0, %bb.nph.us.i12 ], [ %indvar.next.i10, %bb4.us.i ] ; <i64> [#uses=4]
  %.tmp.0.us.i = phi double [ 0.000000e+00, %bb.nph.us.i12 ], [ %11, %bb4.us.i ] ; <double> [#uses=1]
  %scevgep11.i7 = getelementptr [8000 x [8000 x double]]* @A, i64 0, i64 %indvar9.i, i64 %indvar.i6 ; <double*> [#uses=1]
  %scevgep12.i9 = getelementptr [8000 x double]* @s, i64 0, i64 %indvar.i6 ; <double*> [#uses=2]
  %scevgep.i8 = getelementptr [8000 x double]* @p, i64 0, i64 %indvar.i6 ; <double*> [#uses=1]
  %5 = load double* %scevgep12.i9, align 8        ; <double> [#uses=1]
  %6 = load double* %scevgep11.i7, align 8        ; <double> [#uses=2]
  %7 = fmul double %12, %6                        ; <double> [#uses=1]
  %8 = fadd double %5, %7                         ; <double> [#uses=1]
  store double %8, double* %scevgep12.i9, align 8
  %9 = load double* %scevgep.i8, align 8          ; <double> [#uses=1]
  %10 = fmul double %6, %9                        ; <double> [#uses=1]
  %11 = fadd double %.tmp.0.us.i, %10             ; <double> [#uses=2]
  %indvar.next.i10 = add i64 %indvar.i6, 1        ; <i64> [#uses=2]
  %exitcond14 = icmp eq i64 %indvar.next.i10, 8000 ; <i1> [#uses=1]
  br i1 %exitcond14, label %bb6.us.i, label %bb4.us.i

bb.nph.us.i12:                                    ; preds = %bb6.us.i, %bb.i5
  %indvar9.i = phi i64 [ %indvar.next10.i, %bb6.us.i ], [ 0, %bb.i5 ] ; <i64> [#uses=4]
  %scevgep16.i = getelementptr [8000 x double]* @r, i64 0, i64 %indvar9.i ; <double*> [#uses=1]
  %scevgep15.i = getelementptr [8000 x double]* @q, i64 0, i64 %indvar9.i ; <double*> [#uses=2]
  store double 0.000000e+00, double* %scevgep15.i, align 8
  %12 = load double* %scevgep16.i, align 8        ; <double> [#uses=1]
  br label %bb4.us.i

scop_func.exit:                                   ; preds = %bb6.us.i
  %13 = icmp sgt i32 %argc, 42                    ; <i1> [#uses=1]
  br i1 %13, label %bb.i, label %print_array.exit

bb.i:                                             ; preds = %scop_func.exit
  %14 = load i8** %argv, align 1                  ; <i8*> [#uses=1]
  %15 = load i8* %14, align 1                     ; <i8> [#uses=1]
  %16 = icmp eq i8 %15, 0                         ; <i1> [#uses=1]
  br i1 %16, label %bb2.i, label %print_array.exit

bb2.i:                                            ; preds = %bb4.i, %bb.i
  %indvar.i1 = phi i64 [ %indvar.next.i3, %bb4.i ], [ 0, %bb.i ] ; <i64> [#uses=4]
  %i.01.i = trunc i64 %indvar.i1 to i32           ; <i32> [#uses=1]
  %scevgep2.i = getelementptr [8000 x double]* @q, i64 0, i64 %indvar.i1 ; <double*> [#uses=1]
  %scevgep.i2 = getelementptr [8000 x double]* @s, i64 0, i64 %indvar.i1 ; <double*> [#uses=1]
  %17 = load double* %scevgep.i2, align 8         ; <double> [#uses=1]
  %18 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %19 = tail call i32 (%struct._IO_FILE*, i8*, ...)* @fprintf(%struct._IO_FILE* noalias %18, i8* noalias getelementptr inbounds ([8 x i8]* @.str, i64 0, i64 0), double %17) nounwind ; <i32> [#uses=0]
  %20 = load double* %scevgep2.i, align 8         ; <double> [#uses=1]
  %21 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %22 = tail call i32 (%struct._IO_FILE*, i8*, ...)* @fprintf(%struct._IO_FILE* noalias %21, i8* noalias getelementptr inbounds ([8 x i8]* @.str, i64 0, i64 0), double %20) nounwind ; <i32> [#uses=0]
  %23 = srem i32 %i.01.i, 80                      ; <i32> [#uses=1]
  %24 = icmp eq i32 %23, 20                       ; <i1> [#uses=1]
  br i1 %24, label %bb3.i, label %bb4.i

bb3.i:                                            ; preds = %bb2.i
  %25 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %26 = bitcast %struct._IO_FILE* %25 to i8*      ; <i8*> [#uses=1]
  %27 = tail call i32 @fputc(i32 10, i8* %26) nounwind ; <i32> [#uses=0]
  br label %bb4.i

bb4.i:                                            ; preds = %bb3.i, %bb2.i
  %indvar.next.i3 = add i64 %indvar.i1, 1         ; <i64> [#uses=2]
  %exitcond = icmp eq i64 %indvar.next.i3, 8000   ; <i1> [#uses=1]
  br i1 %exitcond, label %bb6.i, label %bb2.i

bb6.i:                                            ; preds = %bb4.i
  %28 = load %struct._IO_FILE** @stderr, align 8  ; <%struct._IO_FILE*> [#uses=1]
  %29 = bitcast %struct._IO_FILE* %28 to i8*      ; <i8*> [#uses=1]
  %30 = tail call i32 @fputc(i32 10, i8* %29) nounwind ; <i32> [#uses=0]
  ret i32 0

print_array.exit:                                 ; preds = %bb.i, %scop_func.exit
  ret i32 0
}
